...
export const loader = async (args: LoaderFunctionArgs) => {
  try {
    ...
    // Get user's recent projects and total count
    let projects = [];
    let actualProjectCount = 0;
    if (user.supabaseUserId) {
      try {
        // Import supabaseAdmin for direct query
        const { supabaseAdmin } = await import('~/lib/database/supabase.server');
        
        // Get exact count from database
        const { count } = await supabaseAdmin.from('projects')
          .select('*', { count: 'exact', head: true })
          .eq('user_id', user.supabaseUserId)
          .eq('status', 'active');
        actualProjectCount = count || 0;
        
        // Get 6 most recent for display
        projects = await db.getUserProjects(user.supabaseUserId, 6);
      } catch (error) {
...
export default function DashboardPage() {
  ...
  // Listen for subscription modal open event from ChatAlert
  useState(() => {
  ...
  return (
    <div className="min-h-screen bg-bolt-elements-background-depth-1 relative">
            ...
            <div className="text-center">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto mb-8">
                ...
                <div className="bg-bolt-elements-background-depth-2 rounded-lg p-6">
                  ...
                  <div className="mt-2">
                    {loaderData.limits?.maxProjects === -1 ? (
                      <p className="text-xs text-bolt-elements-textSecondary">Unlimited</p>
                    ) : (
                      <UsageProgressBar
                        label=""
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-bolt-elements-textPrimary">
                        ...
                        {loaderData.limits?.maxAiGenerations === -1 ? '' : `/${loaderData.limits?.maxAiGenerations || 100}`}
                      </p>
                    </div>
                    <div className="i-ph:devices text-3xl text-green-600" />
                  </div>
                  <div className="mt-2">
                    {loaderData.limits?.maxAiGenerations === -1 ? (
                      <p className="text-xs text-bolt-elements-textSecondary">Unlimited</p>
                    ) : (
                      <UsageProgressBar
                        label=""
                ...
                <div className="bg-bolt-elements-background-depth-2 rounded-lg p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-bolt-elements-textSecondary">Figma Exports</p>
                      <p className="text-2xl font-bold text-bolt-elements-textPrimary">
                        {loaderData.limits?.figmaExportsCount || 0}
                      </p>
                    </div>
                    <div className="i-ph:figma-logo text-3xl text-orange-600" />
                  </div>
                  <div className="mt-2">
                    {loaderData.limits?.maxFigmaExports === -1 || loaderData.limits?.maxFigmaExports === 0 ? (
                      <p className="text-xs text-bolt-elements-textSecondary">
                        {loaderData.limits?.maxFigmaExports === 0 ? 'Upgrade to export' : 'Unlimited'}
                      </p>
                    ) : (
                      <UsageProgressBar
                        label=""
                        used={loaderData.limits?.figmaExportsCount || 0}
                        max={loaderData.limits?.maxFigmaExports || 10}
                      />
                    )}
                  </div>
                </div>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span
                      className={`font-medium capitalize ${
                        loaderData.user?.subscriptionTier === 'enterprise_plus' ? 'text-purple-600' :
                        loaderData.user?.subscriptionTier === 'enterprise' ? 'text-orange-600' :
                        loaderData.user?.subscriptionTier === 'pro' ? 'text-yellow-600' : 'text-blue-600'
                      }`}
                    >
                      {loaderData.user?.subscriptionTier === 'enterprise_plus' ? 'Studio Dominator' :
                       loaderData.user?.subscriptionTier === 'enterprise' ? 'Team Rocket' :
                       loaderData.user?.subscriptionTier === 'pro' ? 'Indie Accelerator' :
                       'Zero-Risk'}
                      {loaderData.user?.subscriptionTier === 'enterprise_plus' && <span className="ml-1 text-xs">👑</span>}
                      {loaderData.user?.subscriptionTier === 'enterprise' && <span className="ml-1 text-xs">🚀</span>}
      ...
      {/* Global event listener for opening subscription modal */}
      <script
        dangerouslySetInnerHTML={{
          __html: `
            window.addEventListener('openSubscriptionModal', function() {
              console.log('Opening subscription modal from event');
            });
          `,
        }}
      />
