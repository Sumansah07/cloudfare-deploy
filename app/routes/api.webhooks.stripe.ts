 ...
 */
function getPriceTier(priceId: string): 'free' | 'pro' | 'enterprise' | 'enterprise_plus' {
  const proPrice = process.env.STRIPE_PRICE_ID_PRO;
  const enterprisePrice = process.env.STRIPE_PRICE_ID_ENTERPRISE;
  const enterprisePlusPrice = process.env.STRIPE_PRICE_ID_ENTERPRISE_PLUS;
  ...
  if (priceId === enterprisePrice) {
    console.log('Matched: enterprise tier');
    return 'enterprise';
  }

  if (priceId === enterprisePlusPrice) {
    console.log('Matched: enterprise_plus tier');
    return 'enterprise_plus';
  }

  console.warn('No tier match found, defaulting to free');
  return 'free';
}
 ...
 */
function mapStripeStatus(stripeStatus: Stripe.Subscription.Status): 'active' | 'cancelled' | 'past_due' | 'paused' {
  switch (stripeStatus) {
    case 'active':
    case 'trialing':
      return 'active';
