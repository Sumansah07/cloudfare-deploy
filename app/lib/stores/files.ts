...
// Buffer is provided globally in entry.client.tsx
...
export class FilesStore {
  ...
  /**
   * Map of files that matches the state of the file system.
   */
  ...
  async saveFile(filePath: string, content: string) {
    // This method is now a no-op because webcontainer is removed.
    // We will just update the in-memory store.
    ...
    logger.info('File updated in memory');
  }

  async #init() {
    // This is now a no-op because webcontainer is removed.
    logger.info('FilesStore initialized without webcontainer.');
  }
  ...
  async createFile(filePath: string, content: string | Uint8Array = '') {
    ...
    if (isBinary) {
      const base64Content = globalThis.Buffer.from(content).toString('base64');
      this.files.setKey(filePath, {
        type: 'file',
    ...
    this.#size++;
    logger.info(`File created in memory: ${filePath}`);
    return true;
  }
  ...
  async createFolder(folderPath: string) {
    ...
    logger.info(`Folder created in memory: ${folderPath}`);
    return true;
  }
  ...
  async deleteFile(filePath: string) {
    ...
    logger.info(`File deleted in memory: ${filePath}`);
    return true;
  }
  ...
  async deleteFolder(folderPath: string) {
    ...
    this.#persistDeletedPaths();
    logger.info(`Folder deleted in memory: ${folderPath}`);
    return true;
  }
...
function convertToBuffer(view: Uint8Array): any {
  return globalThis.Buffer.from(view.buffer, view.byteOffset, view.byteLength);
}
