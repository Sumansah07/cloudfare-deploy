...
export async function checkUsageLimits(args: LoaderFunctionArgs | ActionFunctionArgs) {
  try {
    ...
    try {
      ...
      // Get tier-based limits (configurable via env with fallback to defaults)
      const tier = (supabaseUser as any).subscription_tier || 'free';
      const tierLimits: Record<string, { projects: number; screens: number; storage: number; figmaExports: number }> = {
        free: { 
          projects: parseInt(process.env.TIER_FREE_PROJECTS || '5'), 
          screens: parseInt(process.env.TIER_FREE_SCREENS || '10'), 
          storage: parseInt(process.env.TIER_FREE_STORAGE || '1'),
          figmaExports: parseInt(process.env.TIER_FREE_FIGMA_EXPORTS || '0')
        },
        pro: { 
          projects: parseInt(process.env.TIER_PRO_PROJECTS || '20'), 
          screens: parseInt(process.env.TIER_PRO_SCREENS || '100'), 
          storage: parseInt(process.env.TIER_PRO_STORAGE || '5'),
          figmaExports: parseInt(process.env.TIER_PRO_FIGMA_EXPORTS || '10')
        },
        enterprise: { 
          projects: parseInt(process.env.TIER_ENTERPRISE_PROJECTS || '100'), 
          screens: parseInt(process.env.TIER_ENTERPRISE_SCREENS || '1000'), 
          storage: parseInt(process.env.TIER_ENTERPRISE_STORAGE || '10'),
          figmaExports: parseInt(process.env.TIER_ENTERPRISE_FIGMA_EXPORTS || '100')
        },
        enterprise_plus: { 
          projects: parseInt(process.env.TIER_ENTERPRISE_PLUS_PROJECTS || '200'), 
          screens: parseInt(process.env.TIER_ENTERPRISE_PLUS_SCREENS || '2000'), 
          storage: parseInt(process.env.TIER_ENTERPRISE_PLUS_STORAGE || '20'),
          figmaExports: parseInt(process.env.TIER_ENTERPRISE_PLUS_FIGMA_EXPORTS || '300')
        },
      };
      const currentLimits = tierLimits[tier] || tierLimits.free;

      return {
        projectsCreated: (supabaseUser as any).projects_created || 0,
        maxProjects: currentLimits.projects,
        aiGenerationsCount: actualScreenCount,
        maxAiGenerations: currentLimits.screens,
        storageUsedGB: (((supabaseUser as any).storage_used_bytes || 0) / (1024 * 1024 * 1024)).toFixed(2),
        maxStorageGB: currentLimits.storage,
        figmaExportsCount: (supabaseUser as any).figma_exports_count || 0,
        maxFigmaExports: currentLimits.figmaExports,
        canExportFigma: tier !== 'free',
        canCreateProject,
      };
    } catch (dbError) {
